...
void __cdecl ResetDirtyList(void)
{
    ...
    *(uint16_t *)(uintptr_t)ADDR_DIRTY_PREV_HEAD = 0;
    *(uint16_t *)(uintptr_t)ADDR_DIRTY_TAIL = 0;
}
 ...
 *
 * That is a gap in the harness rather than a fact about this function. The
 * only live-play configuration has its pixel check off by construction, so
 * its evidence is a log of thirteen fixed messages. Verified by reading. */
void __cdecl AddDirtyRect(int32_t left, int32_t top, int32_t right,
                          int32_t bottom)
{
    uint8_t *recs = (uint8_t *)(uintptr_t)ADDR_DIRTY_RECTS;
    uint16_t tail = *(const uint16_t *)(uintptr_t)ADDR_DIRTY_TAIL;
    ...
    *(uint16_t *)(recs + (uint32_t)tail * AM2_DIRTY_RECORD_SIZE
                  + DIRTY_OFF_NEXT) = next;
    *(uint16_t *)(uintptr_t)ADDR_DIRTY_TAIL = next;
}

int mapdraw_install(void)
{
    patch_replace(ADDR_VIEW_UPDATE, (const void *)ViewUpdate, "ViewUpdate", 0);
    patch_replace(ADDR_RESET_DIRTY_LIST, (const void *)ResetDirtyList,
                  "ResetDirtyList", 0);
    patch_replace(ADDR_ADD_DIRTY_RECT, (const void *)AddDirtyRect,
                  "AddDirtyRect", 7);
