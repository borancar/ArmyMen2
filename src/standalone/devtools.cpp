 ...
 *   (FILE absolute -- the game chdirs) and F5/F9 for $TMPDIR/am2-quick.state
 *   (AM2_QUICK_STATE overrides) write and restore the span and the arena. They run on the GAME THREAD, between frames, through the
...
static char            am2_req_reply[256];
static int32_t         am2_req_finished;
static int32_t         am2_hotkey;        /* 1 save, 2 load, set by the host pump */

/* F5/F9's file: AM2_QUICK_STATE, else under TMPDIR. Absolute, because the
 * game chdirs into its own directories as it runs. */
static const char *quick_state_path(void)
{
    static char path[512];
    const char *env = getenv("AM2_QUICK_STATE");
    const char *tmp = getenv("TMPDIR");

    if (env && *env)
        return env;
    if (!path[0])
        snprintf(path, sizeof path, "%s/am2-quick.state", tmp && *tmp ? tmp : "/tmp");
    return path;
}
...
static void devtools_frame(void)
{
    ...
    if (am2_hotkey) {
        ...
        if (am2_hotkey == 1)
            state_save(quick_state_path(), msg, sizeof msg);
        else
            state_load(quick_state_path(), msg, sizeof msg);
        hooklog("devtools: %s", msg);
        am2_hotkey = 0;
    }
}
...
extern "C" void devtools_init(void)
{
    ...
    hooklog("devtools: arena at %p, savestates on `snap save|load FILE`, F5/F9 for %s",
            (void *)AM2_ARENA_BASE, quick_state_path());
}
