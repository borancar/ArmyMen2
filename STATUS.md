...
Last updated: **2026-08-21**, at `e3712cd`. Working tree clean.
...
| | | how |
|---|---:|---|
| `patch_replace` sites | 307 | `grep -rc patch_replace src/game` |
| distinct addresses reconstructed | 307 | 300 of them below the CRT line |
...
| pure unreconstructed leaves | 5 | `tools/vectors.py --all` |
...
| `make selftest` | current | **7,060** vectors, 15,228 words, 13,956 lines, 9,062 spine, 198 variable -- 0 fail |
...
2. **The pure-leaf pool is down to two real entries**: `0x00439cc0` (160B) and
   `0x00423ee0` (256B). `0x0045caa0` and `0x00427974` are the two known false
   positives. After those, offline vectors run out and verification goes back
   to reading plus `tools/ab.sh`.
