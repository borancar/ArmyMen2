...
living in .bss, each a {kind, size} pair, scattered across
`0x004F48E8..0x004FC8A8` with no stride between them.

**The kind is not unique**: four records take kind 0x0B at sizes 0x400, 0x14,
0x14 and 0x400, so a kind selects a handler and the record says which
conversation it belongs to.

Verified by reading the records out of the RUNNING game rather than by a
counter -- `ADDR_MSG_CHAT` holds kind 3 size 0x108, `ADDR_MSG_COLOR` kind 9
size 0x0C, `0x004FC3B8` kind 8 size 0x238, all three exactly as the table says.
...
are exact twins, 95 instructions each, differing in four places once branch
targets are normalised, so they share a body here and the diff is the argument
for it.

They are the most heavily exercised thing this session by a wide margin:
**111,492 and 111,488 calls** on one Boot Camp mission, and the difference of
four is the objects currently holding a footprint down.
...
line (0x0045C000) patched**. Measured: **1,180 of 1,239** entries in
`docs/functions.tsv` below that address have a patch inside them -- so 59
outstanding, which is 1,239 minus 1,180 -- from 1,349 patched addresses. That figure counts merged entries generously and is a
...
59 entries outstanding start at **480 bytes**, and the median is over 1,000.
